...
rbenv install $LATEST_VERSION -s > /dev/null
rbenv global $LATEST_VERSION > /dev/null 2>&1

mkdir -p "$(rbenv root)"/plugins
if [ ! -d "$(rbenv root)/plugins/rbenv-which-ext" ]; then
    git clone https://github.com/yyuu/rbenv-which-ext.git "$(rbenv root)/plugins/rbenv-which-ext" > /dev/null
fi
...
if [ ! -d "$(rbenv root)/plugins/rbenv-gem-migrate" ]; then
    git clone https://github.com/nabeo/rbenv-gem-migrate.git "$(rbenv root)/plugins/rbenv-gem-migrate" > /dev/null
fi
