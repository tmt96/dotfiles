...
pyenv install $LATEST_VERSION_PY2 -s > dev/null 2>&1

LATEST_VERSION_PY3=$(pyenv install -l | egrep "^[[:space:]]*3.[0-9]+.[0-9]+$" | tail -1)
LATEST_VERSION_PY3=${LATEST_VERSION_PY3// /}
pyenv install $LATEST_VERSION_PY3 -s > /dev/null 2>&1

pyenv global $LATEST_VERSION_PY2 $LATEST_VERSION_PY3 > dev/null 2>&1

if [ ! -d "$(pyenv root)/plugins/pyenv-default-packages" ]; then
    git clone https://github.com/jawshooah/pyenv-default-packages.git $(pyenv root)/plugins/pyenv-default-packages > /dev/null 2>&1
fi
if [ ! -d "$(pyenv root)/plugins/pyenv-update" ]; then
    git clone https://github.com/pyenv/pyenv-update.git $(pyenv root)/plugins/pyenv-update > /dev/null 2>&1
fi
